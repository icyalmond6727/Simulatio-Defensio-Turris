...
class PauseMenuUI(BasePopupUI):
    ...
    def __init__(self):
        """
        Initializes the panel size and positional tracking for standard pause menu buttons.
        """
        ...
        rect_left = pygame.Rect(self.menu_rect.left + get_val_x(40), self.menu_rect.bottom - get_val_y(80), btn_w, btn_h)
...
class DefeatMenuUI(BasePopupUI):
    ...
    def __init__(self):
        """
        Initializes the panel size and positional tracking for defeat choices.
        """
    ...
    def draw(self, surface):
        """
        Renders the defeat screen options.
        """
        self.draw_base(surface)
        self.restart_btn.draw(surface)
        self.main_menu_btn.draw(surface)
...
class VictoryMenuUI(BasePopupUI):
    ...
    def __init__(self):
        """
        Initializes the panel size and positional tracking for victory choices.
        """
    ...
    def draw(self, surface):
        """
        Renders the victory screen options.
        """
        self.draw_base(surface)
        self.restart_btn.draw(surface)
...
class NewEnemyMenuUI(BasePopupUI):
    ...
    def __init__(self):
        """
        Initializes a specialized large panel dimension to fit detailed enemy stats.
        """
        ...
        ok_rect = pygame.Rect(
            self.menu_rect.right - get_val_x(config.UI_GAP) - ok_btn_w, 
            self.menu_rect.bottom - get_val_y(config.UI_GAP) - ok_btn_h, 
            ok_btn_w, ok_btn_h
        )
        sys_font = get_font(config.FONT_SYS_SIZE, name = config.FONT_NAME)
    ...
    def draw(self, surface, enemy_name, enemy_data):
        """
        Draws the new enemy alert, rendering both visual avatar and statistical data.
        
        Args:
            surface (pygame.Surface): The rendering target.
            enemy_name (str): The newly encountered enemy's formal string name.
            enemy_data (dict): The enemy's raw statistics payload.
        """
        ...
        surface.blit(name_txt, (self.menu_rect.centerx - name_txt.get_width() / 2, self.menu_rect.top + get_val_y(55)))

        ui_tile_size = get_val_x(config.UI_ICON_SIZE)
        icon_rect = pygame.Rect(self.menu_rect.left + get_val_x(config.UI_PADDING), self.menu_rect.top + get_val_y(120), ui_tile_size, ui_tile_size)
        ...
        if enemy_data:
            ...
            start_y = icon_rect.top - get_val_y(5)
            gap_y = get_val_y(28)
            stats = [
            ...
            for i, stat in enumerate(stats):
                ...
                surface.blit(txt, (stats_x, start_y + i * gap_y))
